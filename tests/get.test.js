...
describe('get.js – Unit tests (Should)', () => {
  ...
  test('returns undefined when path does not exist and no default is provided', () => {
    expect(get(object, 'missing.prop')).toBeUndefined();
    expect(get(object, 'a[1].b')).toBeUndefined();
  });

  test('handles numeric indices in array paths', () => {
  ...
  test('returns undefined when object is null/undefined and no default is provided', () => {
    expect(get(null, 'a.b')).toBeUndefined();
    expect(get(undefined, 'a.b')).toBeUndefined();
  });

  test('handles non-object targets along the path gracefully', () => {
    const data = { a: 5 };
