...
describe('isEmpty.js – Manual tests from Part 1 Test Plan (Group 24)', () => {
  test('EMP-001: Verify detection of empty string', () => {
    expect(isEmpty('')).toBe(true);
  });

  test('EMP-002: Whitespace-only string is NOT treated as empty', () => {
    expect(isEmpty('   ')).toBe(false);
    expect(isEmpty('\t\n\r')).toBe(false);
  });

  test('EMP-003: Zero-width characters are NOT treated as empty', () => {
    expect(isEmpty('\u200B')).toBe(false);
    expect(isEmpty('\u2060')).toBe(false);
  });

  test('EMP-004: Accept valid non-empty string', () => {
    expect(isEmpty('asd asd asd')).toBe(false);
    expect(isEmpty(' potato ')).toBe(false);
  });

  test('EMP-005: Check behavior for empty and “almost empty” collections', () => {
    expect(isEmpty([])).toBe(true);
    expect(isEmpty({})).toBe(true);
...
describe('isEmpty.js – AI-generated test (Grok 4)', () => {
  ...
  test('EMP-AI-004: treats boolean, number, function as empty (no own enumerable properties)', () => {
...
describe('isEmpty.js – Additional exploratory tests', () => {
  ...
  test('EMP-007: considers prototype properties irrelevant', () => {
    function Foo() {}
    Foo.prototype.x = 10;
    expect(isEmpty(new Foo())).toBe(true);
  });

  test('EMP-008: handles symbols as keys as empty', () => {
    const sym = Symbol('test');
    expect(isEmpty({ [sym]: 123 })).toBe(true);
  });

  test('EMP-009: numeric edge cases: NaN, Infinity, -Infinity are treated as empty', () => {
    expect(isEmpty(NaN)).toBe(true);
    expect(isEmpty(Infinity)).toBe(true);
    expect(isEmpty(-Infinity)).toBe(true);
  });

  test('EMP-010: nested structures are not empty if there is a key', () => {
  ...
  test('EMP-012: string containing both zero-width and visible characters is NOT empty', () => {
  ...
  test('EMP-016: consistency: isEmpty(x) === isEmpty(Object(x)) for primitives', () => {
  ...
  test('EMP-017: object with non-enumerable properties is still treated as empty', () => {
    const obj = {};
    Object.defineProperty(obj, 'x', { value: 10, enumerable: false });
    expect(isEmpty(obj)).toBe(true);
  });

  test('EMP-018: treats File objects or Blobs as empty (no enumerable props)', () => {
